...
function jarvisMotionAcceptFix(lat,lon,speedKmh,accuracyM){
  ...
  const visualThreshold=Math.max(58,Math.min(78,50+acc*1.35));
  ...
  const departureHeadingMin=Math.max(VISUAL_ESCAPE_HEADING_DEG,82);
  ...
  const departureIntent=jarvisDepartureFixes>=4;
